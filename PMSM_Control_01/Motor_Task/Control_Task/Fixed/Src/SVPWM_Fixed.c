...
{
    uint8_t sector = 0;
    q31_t V1 = (q31_t)U_beta;
    q31_t V2 = 28377 * U_alpha - 16384 * U_beta;
    q31_t V3 = -28377 * U_alpha - 16384 * U_beta;
    if (V1 > 0) sector |= 1;
    if (V2 > 0) sector |= 2;
    if (V3 > 0) sector |= 4;    
    *N = sector;
    
    factor = ((56756 * (q31_t)T_s)) / (q31_t)V_dc;  
    X = factor * (q31_t)U_beta;
    Y = factor * ((28378 * (q31_t)U_alpha + 16384 * (q31_t)U_beta) >> 15);
    Z = factor * ((-28378 * (q31_t)U_alpha + 16384 * (q31_t)U_beta) >> 15);
    // q31_t T1, T2;
