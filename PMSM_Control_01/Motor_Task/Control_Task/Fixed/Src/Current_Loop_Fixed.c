...
/// @param B
/// @param C
inline void Phase_Min_Max_Fixed(Motor_Control_t *pControl, float A, float B, float C)
{
    Current_Loop_Fixed_t *pCurrent_Loop_Fixed = (Current_Loop_Fixed_t *)pControl->Current_Loop.pCurrent_Loop;
    Speed_Loop_Fixed_t *pSpeed_Loop = (Speed_Loop_Fixed_t *)pControl->Speed_Loop.pSpeed_Loop;
...
void Dead_Zone_Compensation_Fixed(Motor_Control_t *pControl, q31_t we, q31_t theta)
{
    Current_Loop_Fixed_t *pCurrent_Loop_Fixed = (Current_Loop_Fixed_t *)pControl->Current_Loop.pCurrent_Loop;
    q31_t Ialpha_tmp, Ibeta_tmp;    
    q31_t Ia_pre = 0, Ib_pre = 0, Ic_pre = 0;
    q15_t theta_comp = theta + we * pCurrent_Loop_Fixed->Loop_time_s * (2.0f);
    q31_t Dead_Time_Duty = (Dead_TIME_S) / pCurrent_Loop_Fixed->Loop_time_s;

    Ialpha_tmp = pCurrent_Loop_Fixed->Id_fb * arm_sin_q15(theta_comp) - pCurrent_Loop_Fixed->Iq_fb * arm_cos_q15(theta_comp);
    Ibeta_tmp = pCurrent_Loop_Fixed->Id_fb * arm_cos_q15(theta_comp) + pCurrent_Loop_Fixed->Iq_fb * arm_sin_q15(theta_comp);
...
void Current_Loop_Run_Fixed(Motor_Control_t *pControl)
{
    Current_Loop_Fixed_t *pCurrent_Loop_Fixed = (Current_Loop_Fixed_t *)pControl->Current_Loop.pCurrent_Loop;
    Speed_Loop_Fixed_t *pSpeed_Loop = (Speed_Loop_Fixed_t *)pControl->Speed_Loop.pSpeed_Loop;
    struct EffFluxObserver_Parameter *pObserver = (struct EffFluxObserver_Parameter *)pControl->pObserver;
...
void Current_IDLE_TASK_Fixed(Motor_Control_t *pControl)
{
    ...
    // Code for CURRENT_IDLE state
    if (pSystem_Loop->Run_flag == 1)
    {
        pControl->Current_Loop.Status = CURRENT_READY;
    }
    else
    {
        pControl->Current_Loop.PWM_OPEN_Flag = PWM_CLOSE;
...
void Current_READY_TASK_Fixed(Motor_Control_t *pControl)
{
    ...
    if (pSystem_Loop->Run_flag == 1)
    {
        ...
        else
        {
            ...
            pControl->Current_Loop.Status = CURRENT_OFFSET_CHECK;
        }
    }
    else
    {
...
void Current_OFFSET_CHECK_TASK_Fixed(Motor_Control_t *pControl)
{
    ...
    if (pSystem_Loop->Run_flag == 1)
    {
        ...
        {
            // Code for CURRENT_OFFSET_CHECK state
            ...
            if (pCurrent_Loop_Fixed->offset_check_cnt >= MOTOR_ADC_OFFSET_SAMPLE_CNT)
            {
                ...
                {
                    pControl->Current_Loop.Status = CURRENT_RUN;
...
void Current_RUN_TASK_Fixed(Motor_Control_t *pControl)
{
    ...
    else
    {
        ...
        {
            Current_Loop_Run_Fixed(pControl);
            pControl->Current_Loop.PWM_OPEN_Flag_z = pControl->Current_Loop.PWM_OPEN_Flag;
        }
    }
    return;
