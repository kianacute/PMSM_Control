...
# -*- coding: utf-8 -*-
"""
从 Motor_Para/Motor_Parameters.csv 生成查表头文件 Motor_Lookup_Tables.h。

CSV 中存储的是**调谐参数**（如阻尼系数、带宽比），
脚本根据电机参数和公式计算出最终的 PI/PLL 增益值。

输出:
  Motor_Task/Control_Task/Float/Inc/Motor_Lookup_Tables.h  （Keil 工程引用）
  Motor_Task/Control_Task/Float/Src/Motor_Lookup_Tables.h  （同步副本）
...
OUTPUT_FILENAME = "Motor_Lookup_Tables.h"
...
def gen_motor_param_macros(motor_params):
    ...
    # 未映射参数告警，防止新增参数被静默丢弃
    for csv_name in motor_params:
        if csv_name in SCALAR_MACROS or csv_name in SCALAR_IGNORED:
            continue
        print(f"  [WARN] MotorBase param '{csv_name}' not mapped to any macro, ignored")
